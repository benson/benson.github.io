...
export const LOC_ICONS = {
  ...
  container: '<svg class="loc-icon" viewBox="0 0 14 14" aria-hidden="true"><rect x="2" y="3" width="10" height="8" rx="1" fill="none" stroke="currentColor" stroke-width="1"/><path d="M4 5.5h6M4 8.5h6" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>',
...
export function locationPillHtml(loc, { withRemove = false, index = -1 } = {}) {
  ...
  if (!n) return '';
  const icon = LOC_ICONS[n.type] || LOC_ICONS.container;
...
export function containerEditRowHtml(c, allowedTypes = LOCATION_TYPES) {
  ...
  const typeRadiosHtml = allowedTypes.map(t => `<label class="loc-type-radio${t === c.type ? ' is-selected' : ''}">
    ...
    <span class="loc-pill loc-pill-${esc(t)}">${LOC_ICONS[t] || LOC_ICONS.container}<span>${esc(t)}</span></span>
  </label>`).join('');
