...
// ============ Per-set metadata (loaded from booster-data/metadata.json) ============
// Source: https://bensonperry.com/booster-data/metadata.json (auto-generated by new-set workflow)
// On fetch failure we fall back to the snapshot below — a stale snapshot is far better than
// silently losing Special Guests / bonus-sheet data for every set. Snapshot is refreshed
...
export const SPECIAL_GUESTS_RANGES = Object.fromEntries(
  Object.entries(_metadata.sets || {})
    .filter(([, v]) => v.specialGuests)
    .map(([k, v]) => [k, v.specialGuests])
);

// Sets with separate "bonus sheet" sets that appear in boosters (main set code -> bonus set code)
export const BONUS_SHEET_SETS = Object.fromEntries(
  Object.entries(_metadata.sets || {})
    .filter(([, v]) => v.bonusSheet)
    .map(([k, v]) => [k, v.bonusSheet])
);

// Sets with The Big Score bonus sheet
export const SETS_WITH_BIG_SCORE = new Set(
  Object.entries(_metadata.sets || {})
    .filter(([, v]) => v.hasBigScore)
    .map(([k]) => k)
);
